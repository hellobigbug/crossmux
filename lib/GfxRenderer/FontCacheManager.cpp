...
bool FontCacheManager::canIdlePrewarm(const int fontId) const { return sdCardFonts_.count(fontId) != 0; }
...
bool FontCacheManager::needsPrewarmScan(const int fontId) const {
  ...
  for (uint8_t style = 0; style < 4; style++) {
    const EpdFontData* data = family->second.getData(static_cast<EpdFontFamily::Style>(style));

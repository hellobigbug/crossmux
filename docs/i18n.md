...
Unified firmware stores `contentProfile` separately from `language`. Legacy
`langSku=cn/global` maps directly; without that marker only `ZH_CN` maps to
China. Changing UI language later never changes the profile or service host.
The selector lists all 33 generated languages.
